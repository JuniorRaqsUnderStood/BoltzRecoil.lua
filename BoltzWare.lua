...
-- Add-on: Config Management UI
local ConfigTab = Window.NewPage("config")
local ConfigSection = ConfigTab.NewSection("Configurations")

local configFolder = "BoltzWare_Configs"
...
local ConfigDropdown = ConfigSection.Dropdown("Select Config", listConfigs(), function(value)
    selectedConfig = value
end)

ConfigSection.Button("Create Config", function()
    MainUI:Prompt("Enter config name:", function(input)
        if input and input:match("%w") then
            selectedConfig = input
            local encoded = HttpService:JSONEncode(EncodeConfig(Settings))
            writefile(string.format("%s/%s.json", configFolder, input), encoded)
            ConfigDropdown:SetOptions(listConfigs())
            ConfigDropdown:SetValue(input)
        else
            MainUI:Notify("Invalid name.")
        end
    end)
end)
...
ConfigSection.Button("Save Config", function()
    if selectedConfig then
        local encoded = HttpService:JSONEncode(EncodeConfig(Settings))
        writefile(string.format("%s/%s.json", configFolder, selectedConfig), encoded)
        MainUI:Notify("Saved: " .. selectedConfig)
    else
        MainUI:Notify("No config selected.")
    end
end)

ConfigSection.Button("Load Config", function()
    if selectedConfig then
        local path = string.format("%s/%s.json", configFolder, selectedConfig)
        if isfile(path) then
            local data = readfile(path)
            local success, decoded = pcall(function()
                return DecodeConfig(HttpService:JSONDecode(data))
            end)
            if success then
                Settings = decoded
                MainUI:Notify("Loaded: " .. selectedConfig)
            else
                MainUI:Notify("Failed to load.")
            end
        else
            MainUI:Notify("Config not found.")
        end
    else
        MainUI:Notify("Select a config.")
    end
end)

-- End of Config Management Add-on
